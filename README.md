...
### v0.1.9 (当前) - 代码重构与稳定性增强

**🎉 重磅更新：架构优化与并发保护**

- ✅ **main.py 重构**：将 1000+ 行代码拆分为模块化结构 (`ollamapilot/cli/`)
  - `session.py` - 会话数据类
  - `completer.py` - 命令自动补全
  - `chat_manager.py` - 聊天管理器核心逻辑
- ✅ **Ollama 并发保护**：新增全局锁机制，防止生成模型和向量模型冲突
- ✅ **断点续传**：索引失败后可使用 `/resume` 命令恢复任务
- ✅ **命令自动补全**：Tab 键补全命令（如 `/h` → `/help`）
- ✅ **默认知识库**：`/index` 无参数时默认索引 `./knowledge_base` 文件夹
- ✅ **异步后台索引**：聊天和索引可同时进行，互不阻塞
- ✅ **技能触发优化**：知识库和 Web 搜索改为显式触发，避免误触发
