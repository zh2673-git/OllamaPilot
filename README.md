...
### v0.3.5 (当前) - 智能上下文管理与动态分块优化

**🎉 自动上下文检测 + 智能分块 + 工具截断优化**

- ✅ **自动上下文检测**：新增 `model_context.py` 模块，自动扫描 Ollama 模型并检测上下文窗口大小
- ✅ **动态 num_ctx 配置**：支持 `CHAT_NUM_CTX=auto` 模式，自动匹配模型最优上下文大小
- ✅ **智能工具截断**：工具输出截断阈值基于实际 `num_ctx` 动态计算，避免超出上下文限制
- ✅ **Embedding 动态分块**：根据模型上下文窗口自动调整文档分块大小（2K→8K）
- ✅ **索引速度提升**：分块数量减少 74%，索引速度提升约 3 倍
- ✅ **性能平衡优化**：大模型（256K）限制在 32K 使用，兼顾速度与功能
