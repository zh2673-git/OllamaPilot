...
<p align="center">
  <strong>🚀 本地 AI 智能助手框架 v0.6.0</strong><br>
  <em>LangChain 1.0+ × Ollama 小模型 × GraphRAG 知识图谱 × Harness 增强架构 × 零 API 网络搜索 × OpenCode 式终端交互 × Channels 多渠道远程控制</em>
</p>
...
│                          ▼                                              │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │                OllamaPilotHarnessAgent (包装层)                   │   │
│  │  ┌─────────────────────────────────────────────────────────┐    │   │
│  │  │              MiddlewareChain (中间件链)                    │    │   │
│  │  │  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐        │    │   │
│  │  │  │   Context   │ │   Memory    │ │  Sandbox    │        │    │   │
│  │  │  │  Injection  │ │  Retrieval  │ │   Prepare   │        │    │   │
│  │  │  └──────┬──────┘ └──────┬──────┘ └──────┬──────┘        │    │   │
│  │  │         │               │               │               │    │   │
│  │  │         ▼               ▼               ▼               │    │   │
│  │  │  ┌─────────────────────────────────────────────────┐    │    │   │
...
### v0.6.0 - Harness 增强架构（重大更新）

**🎉 重磅更新：Harness 增强架构 - 中间件链 + 三层工具 + 子 Agent 委托**

#### 🏗️ Harness 架构（增强而非替换）

全新 Harness 架构，在保留 OllamaPilot 核心优势的基础上，借鉴 DeerFlow 和 Claude Code 的设计：

| 层级 | 组件 | 功能 | 状态 |
|------|------|------|------|
| **中间件链** | MiddlewareChain | 链式编排现有中间件，支持条件中断 | ✅ 默认启用 |
| **三层工具** | Tool 基类 + 适配器 | 完整生命周期管理（校验/权限/执行/渲染） | ✅ 默认启用 |
| **沙箱系统** | Sandbox | Local/Docker 安全执行环境 | ⚠️ 可选启用 |
| **记忆增强** | EnhancedMemory | LLM 事实提取叠加到现有记忆 | ⚠️ 可选启用 |
| **子 Agent** | TaskTool | 任务分解 + 并发执行（max 3） | ✅ 默认启用 |

**核心设计原则**：
- ✅ **增强而非替换**：保留四层 Context、Skill-Middleware、文件驱动记忆、强制回复保底
- ✅ **100% 向后兼容**：现有 Skill 和工具无需修改
- ✅ **渐进式升级**：可开关，不影响现有功能
...
```python
# 中间件执行顺序
用户输入 → ContextInjection → MemoryRetrieval → Compaction → Clarification → Agent
                ↓                    ↓              ↓              ↓
         调用ContextBuilder   调用MemoryManager  调用Compactor   条件中断检查
```

**中间件列表**：
- ✅ `ContextInjectionMiddleware` - 注入四层 Context
- ✅ `MemoryRetrievalMiddleware` - 检索相关记忆
- ✅ `CompactionMiddleware` - 上下文压缩
- ✅ `ClarificationMiddleware` - 澄清请求（支持条件中断）

#### 🛠️ 三层工具架构

现有 Skill、适配器、新 Tool 基类三层共存：

| 层级 | 组件 | 说明 |
|------|------|------|
| Layer 1 | 现有 Skill | @skill 装饰器，100% 兼容 |
| Layer 2 | SkillToolAdapter | 自动转换现有 Skill |
| Layer 2 | BuiltinToolAdapter | 包装 @tool 装饰的工具 |
| Layer 3 | Tool 基类 | 完整生命周期（validate → check_permission → execute → render_result）|

**Tool 生命周期**：
```python
Tool.run() → validate() → check_permission() → execute() → render_result()
```

#### 🤖 子 Agent 委托（TaskTool）

借鉴 DeerFlow 设计，支持复杂任务分解：

- ✅ **自动任务分解**：根据描述智能分解为子任务
- ✅ **并发执行**：最多 3 个子 Agent 并行执行
- ✅ **结果整合**：自动合并子任务结果
- ✅ **内置子 Agent**：通用、代码、研究三种专家

#### 📊 版本信息动态注入

System Prompt 最开头动态注入版本信息，确保 LLM 始终报告正确版本：

```markdown
# SYSTEM - 系统信息（最高优先级）
## 当前版本
你正在运行 OllamaPilot v0.6.0。
当用户询问"你是谁"或"你的版本"时，必须回答 v0.6.0，优先使用此信息而非历史对话。
```
...
| 阶段 | 优化前 | 优化后 |
|------|--------|--------|
| Chunk embedding | 串行处理 | ✅ 多线程并行 |
| Entity embedding | 多线程并行 | ✅ 多线程并行 |
| Relation embedding | 多线程并行 | ✅ 多线程并行 |

- ✅ **Chunk 向量化并行**：文档切分块（66-135块）现在支持多线程并行生成向量
- ✅ **全流程并行**：Chunk、Entity、Relation 三个阶段全部实现 ThreadPoolExecutor 并行
- ✅ **智能线程管理**：每个阶段独立线程池，避免资源竞争

#### 🚀 批量写入优化

**彻底解决大文档保存卡顿问题（3分钟 → 几秒）**：

- ✅ **禁用自动持久化**：批量模式禁用每次写入自动保存，大幅减少 IO 操作
- ✅ **一次性 flush**：收集所有数据后一次性调用 `persist()` 写入
- ✅ **O(n²) → O(n)**：修复 SimpleVectorStore 的重复写入问题
...
- ✅ **动态并行数**：初始 50 workers，速度 < 3 批/秒时自动 +30（最高 200）
- ✅ **动态批次调整**：workers 触顶后自动增加 batch_size（10 → 30）
- ✅ **步进优化**：workers 每次 +30 而非 +10，更快达到最优值

**自适应机制**：
```
初始: workers=50, batch_size=10
      ↓ 速度 < 3 批/秒
workers=80 → 110 → 140 → 170 → 200
      ↓ workers 触顶
batch_size=30 → 50
```
...
**SimpleVectorStore 优化**：
- 添加 `_auto_persist` 开关，控制是否每次 add 都写文件
- 添加 `set_auto_persist()` 和 `persist()` 方法

**TripleVectorStore 优化**：
- `set_auto_save()` 同步所有底层 store 的自动保存开关
- `flush()` 方法手动触发一次性保存

---

### v0.5.3 - 断点续传完善与批量索引优化

**🔧 修复关键问题：索引断点续传 + 批量处理卡顿**

#### 🛠️ 断点续传功能修复

**彻底解决大文档索引中断后无法恢复的问题**：

- ✅ **实体抽取进度保存**：每批次完成后立即保存进度，支持中断后精准恢复
- ✅ **Embedding 生成进度保存**：解决了之前 embedding 生成过程不保存进度的问题
- ✅ **变量作用域修复**：修复 `last_batch_start` 变量作用域错误导致的进度计算问题
- ✅ **批次粒度追踪**：精确记录每个批次的开始/结束块索引，确保断点准确
...
**优化 LLM 调用策略，消除 95% 卡顿问题**：

- ✅ **批量 embedding 生成**：每批次的多个块 embedding 一次性生成，减少 API 调用次数
- ✅ **超时保护机制**：5 分钟超时保护，大文档稳定处理不超时
- ✅ **实时进度反馈**：显示批次进度、实体数、预估时间

**性能对比**：
| 文档 | 修复前 | 修复后 | 提升 |
|------|--------|--------|------|
| 伤寒论(上) 66块 | 卡95% | ~2分钟 | ✅ 稳定完成 |
| 伤寒论(下) 135块 | 超时风险 | 预估5分钟 | ✅ 稳定完成 |

#### 🔄 状态管理改进

- ✅ **运行状态持久化**：索引线程状态正确保存到注册表
- ✅ **重启检测机制**：启动时正确识别正在运行的索引任务
- ✅ **进度回调优化**：实体抽取和 embedding 生成各自有独立的进度回调

---

### v0.5.2 - LightRAG 增强与知识图谱升级
