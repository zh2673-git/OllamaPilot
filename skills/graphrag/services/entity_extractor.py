...
class HybridEntityExtractor:
    ...
    def extract_batch(
        ...
        for batch_idx, batch_start in enumerate(range(0, len(chunks), batch_size)):
            ...
            # 批次开始时调用进度回调（让调用方可以记录进度）
            if progress_callback:
                total_entities_so_far = sum(len(r[0]) for r in results)
                progress_callback(batch_idx, total_batches, batch_start, len(chunks), total_entities_so_far)
            ...
            if progress_callback:
                total_entities_so_far = sum(len(r[0]) for r in results)
                progress_callback(batch_idx + 1, total_batches, batch_end, len(chunks), total_entities_so_far)
